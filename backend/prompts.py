...
Валидация change_log (SELF-CORRECTION):
- ПЕРЕД тем как записать изменение в change_log, ПРОВЕРЬ: этот текст реально есть в updated_resume_text?
- Если ты написал "Добавил навык Python", но в тексте резюме слова "Python" нет — это ГАЛЛЮЦИНАЦИЯ. Исправь текст резюме или удали запись из лога.
- KEYWORD STUFFING CHECK: Убедись, что добавленные ключевые слова (ATS) читаются естественно, но они ДОЛЖНЫ быть в тексте.
...
Верни JSON строго в структуре (без лишних ключей):
{
  ...
  "change_log": [
    {
      "checkbox_id": "",
      "what_changed": "Точное описание что изменилось (например: 'Добавлен навык: Docker')",
      "where": "summary | skills | experience | education | other",
      "before_excerpt": null,
      "after_excerpt": "Цитата из НОВОГО текста, подтверждающая изменение (ОБЯЗАТЕЛЬНО)"
    }
  ]
}
