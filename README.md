...
Перед деплоем backend Cloud Build запускает `alembic upgrade head` из нового
...
| `UndefinedColumnError` после деплоя | Backend запущен с кодом, который ожидает новую схему БД, но Alembic-миграция не была применена | Запустите `alembic upgrade head` из окружения с доступом к БД или перезапустите Cloud Build после добавления шага миграции. |
