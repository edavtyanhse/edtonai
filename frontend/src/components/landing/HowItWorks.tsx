...
const HowItWorks: React.FC<HowItWorksProps> = ({ lang }) => {
  ...
  return (
    <section className="py-24 px-4 sm:px-6 lg:px-8 bg-slate-50 dark:bg-slate-900 relative overflow-hidden">
      {/* Decorative gradient */}
      ...
      <div className="container mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-bold text-slate-900 dark:text-white mb-6">{t.title}</h2>
          <p className="text-slate-600 dark:text-slate-400 max-w-2xl mx-auto">{t.description}</p>
        </div>

        <div className="grid md:grid-cols-4 gap-8 relative">
          {/* Connector Line (Desktop) */}
          <div className="hidden md:block absolute top-12 left-[10%] right-[10%] h-0.5 bg-white dark:bg-slate-800 z-0"></div>

          {t.steps.map((step, index) => {
            const Icon = icons[index]
            return (
              <div key={index} className="relative z-10 flex flex-col items-center text-center">
                <div className="w-24 h-24 bg-white dark:bg-slate-800 rounded-full border-4 border-slate-200 dark:border-slate-900 flex items-center justify-center mb-6 shadow-xl relative group">
                  <Icon className="w-8 h-8 text-brand-400 group-hover:scale-110 transition-transform" />
                  <div className="absolute -top-2 -right-2 w-8 h-8 bg-brand-600 rounded-full flex items-center justify-center text-white font-bold text-sm border-4 border-slate-200 dark:border-slate-900">
                    {index + 1}
                  </div>
                </div>
