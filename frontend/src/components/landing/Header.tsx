...
const Header: React.FC<HeaderProps> = ({ lang, toggleLang }) => {
  ...
  return (
    <header
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-slate-900/80 backdrop-blur-lg border-b border-slate-800 py-3' : 'bg-transparent py-5'}`}
    >
        ...
        <div className="hidden md:flex items-center gap-4">
          ...
          {user ? (
            ...
            <>
              <Link
                to="/login"
                className="text-white font-medium text-sm hover:text-brand-300 transition-colors cursor-pointer"
              >
                {t.login}
              </Link>
              <Link
          ...
          {user ? (
            <>
              <div className="flex flex-col gap-2 mt-4">
                ...
                <button
                  onClick={handleLogout}
                  className="text-slate-400 py-2 text-left flex items-center gap-2 justify-center"
                >
